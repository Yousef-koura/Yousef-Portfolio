...
- ~~Portrait use on ABOUT~~ RESOLVED 2026-08-23 (DECISIONS #48): portrait approved for ABOUT as a small inline figure in the opening statement — see PHASE 4C below.
- ~~Military service presentation~~ RESOLVED 2026-08-23 (DECISIONS #49): presented on ABOUT as narrative prose (not a timeline entry) — see PHASE 4C below.
...
Update (2026-08-23, orphaned hero hairline removal — user visual review, see DECISIONS #47):
- Investigation CONFIRMED the hairline sitting between the header and the "Work." H1 as an orphaned artifact of #45's kicker removal: `<div className="border-t border-line" />` at the top of `WorkHero` was the SectionFrame-style top rule whose label had been deleted, leaving a content-less rule floating in whitespace. Site-wide audit found it is the ONLY standalone empty hairline in `src/` (every other `border-t` pairs with a wayfinding label or immediate content), and Home's live hero has NO rule above it — hairlines there begin only at indexed sections. Removed so `/work` flows header → hero → first hairline at the flagship Movenue frame, matching Home's pattern. Hero copy, entrance choreography (#46), top/gap spacing values, and everything below the hero untouched; component doc-comment updated to match reality.
- Verified: production build clean (`/work` static + 7 SSG detail routes), `tsc --noEmit`, ESLint clean; headless suite vs the production server — hero section contains ZERO bordered elements at 1440×900 AND 375×812 (previously 1); flagship Movenue rule, ALL PROJECTS rule, contact closing rule, both footer rules, all six grid-tile borders, and the flagship capture border all intact; zero horizontal overflow at both widths; entrance animation re-verified per-frame — riser observed mid-flight (y-offset >2px) while metadata holds opacity 0, then full settle (riser seated, metadata+intro opaque) at both widths; prerendered SSR HTML carries the hero with no bare rule div before the H1; `/work/movenue` serves 200; Home portal hero/kickers/indexed-section rules regression-clean; zero console/page errors. Reduced-motion A/B against a pre-fix HEAD worktree build proved mount behavior byte-identical before/after (transient hidden state of ~8–22 frames under emulated reduce, settling fully opaque/seated — pre-existing since #46, unchanged by this pass, flagged for awareness). Before/after screenshots captured for manual review (agent image read-back unavailable in this session). Final visual sign-off manual by user.

Update (2026-08-23, footer MENU 2-column grid fix pass — GLOBAL shared-component change, user-directed; NOT scoped to a single page — the Footer renders once in the root layout, so all 13 routes (Home, Work index, all seven `/work/[slug]` detail pages, About, Experience, Publications, Contact) are affected and were verified):
- The footer MENU list (`Footer.tsx`) changed from a single vertical flex column (`mt-5 flex flex-col gap-3`, 6 stacked rows) to a 2-column grid (`mt-5 grid grid-cols-2 grid-flow-col grid-rows-3 gap-x-8 gap-y-3`) to cut vertical height. Column-major fill (`grid-flow-col grid-rows-3`) keeps the visual reading order top-to-bottom-then-left-to-right identical to the site's page order: column 1 = Home/Work/About, column 2 = Experience/Publications/Contact. DOM order is unchanged (items still mapped from `site.nav` in source order; CSS placement never reorders DOM), so keyboard Tab/focus sequence and screen-reader announcement order are byte-identical to before. Layout-only change: type scale, link markup, hover states, `mt-5`, and the vertical rhythm token preserved exactly (`gap-y-3` = the previous `gap-3`; `gap-x-8` standard step) — reuses the DECISIONS #36-established plain `grid-cols-2` Tailwind idiom, no new tokens/radii/motion invented.
- Unaffected by construction and verified intact: identity block (name + bio + location), CONNECT column (still `flex flex-col`, 3 links mailto/LinkedIn/GitHub), legal/build-credit row, footer borders. No page-level file touched.
- Measured A/B (headless Chrome CDP against production server; old layout measured via scoped stash → rebuild → measure → restore): MENU list 204px → 96px on every route at every breakpoint; whole footer height 901 → 793px at 375 (−108) and 466 → 405px at 1440 (−61).
- Verified: production build clean (16 pages incl. all seven `/work/[slug]` SSG routes), `tsc --noEmit` exit 0, ESLint exit 0; prerendered HTML carries the new grid classes on ALL 13 routes + `_not-found` (14/14 files); headless layout matrix 39/39 (13 routes × 375/820/1440): `display:grid`, exactly 2 tracks, `grid-auto-flow:column`, column x-alignment exact with row tops aligned ≤2px, correct per-column vertical ordering, zero horizontal overflow at every width, identity/location/legal/CONNECT assertions pass on every load, zero console/page errors; keyboard verified with REAL CDP key events — full body→footer Tab walks at 1440 pass 13/13 with the exact six-link sequence Home→Work→About→Experience→Publications→Contact, focused walks at 375 and 820 across all 13 routes pass 26/26; DOM-order focusable enumeration confirms sequential-focus order matches site.nav order per HTML spec. Final visual sign-off manual by user.

Open / unresolved:
- PneumoScan data anomalies ("85.1–85%" figure, dual-date timeframe) — user correction requested, rendered verbatim meanwhile.
- Minor pre-existing nuance flagged during verification (not a regression; animation untouched by user direction): under emulated `prefers-reduced-motion`, the hero's hidden variant applies for ~8–22 frames at hydration before settling instantly — imperceptible fade-in rather than a completed rise. A/B-proven identical to the already-reviewed pre-fix build (DECISIONS #47).
- Final visual sign-off of this page by the user.
...
A **user-directed implementation milestone**, sibling to Phases 4A/4B. Implements ONLY the ABOUT page (`/about`). Phase 11's full About/Experience/Publications/Contact scope remains open.

Completed (2026-08-23, see DECISIONS #48–50):
- `/about` rewritten from the Phase-4A placeholder into a five-section page matching WORK's established conventions (same `max-w-6xl` container rhythm, masked line-reveal hero via `MaskedLineReveal`, hairline `SectionFrame` wayfinding, closing forward path):
  - **Hero** — "About." display H1 + quiet mono metadata (Menoufia, Egypt · B.Sc. line), entrance identical to `WorkHero`'s reduce-gated choreography; no top rule (hairlines belong to indexed sections, per DECISIONS #47's site-wide convention).
  - **Opening personal statement** ("Who I am") — human-first framing built ONLY from sourced identity facts (resume-summary substance in first person; Menoufia base; end-to-end AI-driven products interest), accompanied by the approved portrait cutout pair rendered as a **small inline figure (~128px mobile / ~208px desktop)** beside the statement — visually distinct from Home's full-height portal treatment (**DECISIONS #48**, resolving PHASE3_UX §14 item 3).
  - **Trajectory** — one continuous first-person narrative: Mechatronics education (MSA/Greenwich partnership, GPA, Agri-Bot graduation project) → ITI → D-HUB (3rd place ERI challenge woven in at the moment it happened) → PioPetro → **military service as a standalone prose paragraph describing what the role actually involved, NOT formatted as a title/org/dates timeline entry** (**DECISIONS #49**, resolving PHASE3_UX §14 item 5) → FlyRank (current) → Movenue (founder). Two certifications woven contextually inline — ML Specialization (DeepLearning.AI, 2023) at the ITI moment and IBM Containers w/ Docker/Kubernetes/OpenShift (Jun 2026) at FlyRank — **no standalone certifications list anywhere on the page** (**DECISIONS #50**, resolving PHASE3_UX §14 item 6).
  - **Education** — spec-grid of the formal facts: B.Sc. Mechatronics Systems Engineering · MSA University, Giza, in partnership with University of Greenwich UK · Sep 2019–Jul 2024 · GPA 3.62/4.0.
  - **Capabilities** — Home's `<Capabilities />` component reused unchanged (ML & AI / Computer Vision & Robotics / Data & Systems; zero technologies added or removed).
  - **Links out** — LinkedIn + GitHub external QuietLinks, contextual `/work` and `/experience` paths (trajectory ends with "Full timeline and metrics"), ghost "Get in touch" pill → CONTACT; page never dead-ends.
- Content integrity held: every fact/date/figure traces to `yousef-portfolio-content.md` (§1 identity, §3 summaries, §4 experience incl. the military role description, §5 education, §6 Movenue, §9 certifications); unsupported personal details were NOT invented (the source carries no hobbies/origin story — flagged during planning); certifications NOT related to woven moments (Python 101, Arduino, Claude/MCP training, AI Fluency) deliberately left off this page.
- Verified: production build clean (`/about` static prerendered), `tsc --noEmit`, ESLint clean; headless suite against the production server — all sourced facts/dates present in SSR HTML, forbidden strings absent (no cert-list leakage, no "Yalla7gez"), both portrait crops served per breakpoint with correct computed widths (128px@375 / 176px@820 / 208px@1440), zero horizontal overflow at 375/820/1440, single H1, reduced-motion renders hero immediately, Capabilities clusters confirmed rendering on real scroll (8/7/10 items), zero console/page errors; Home portal hero + role line and `/work` + `/work/movenue` regression-clean.

Update (2026-08-23, revision pass — two new confirmed facts + Trajectory restructure + Capabilities logo wall, user-directed; see DECISIONS #51):
- **Who I am:** two biographical facts the user confirmed directly (absent from the content source) added as one quiet body paragraph between the display statement and the Movenue paragraph — born 2002, grew up in the UAE (Sharjah, then Dubai), moved to Egypt in 2019 for university; Emirates Club (UAE) youth academy ages 15–18, left wing. Plain factual statements, no traits derived. CONTENT.md "Identity" updated with both as newly CONFIRMED.
- **Trajectory:** six prose paragraphs restructured into a seven-entry date-column list matching Home's `ExperiencePreview` pattern exactly (mono timeframe left, condensed role/org/highlight right, hairline rows, staggered reveals; champagne + Current badge on both Jun 2026 entries). Condensing lost zero facts/dates/metrics — every figure spot-checked against the previous version (Greenwich partnership, 3.62/4.0 GPA, Agri-Bot 20 FPS/96%/Jetson Nano/IUGRC 2024, ITI 6+/97% F1 + ML Specialization inline, D-HUB 96% + 3rd place ERI challenge, PioPetro 4+/87% R² across exploration/artificial lift/well stimulation, full military-role description, FlyRank temporal split/Decision Tree/rule-based baseline + IBM Containers cert, Movenue stack/link). Movenue became its own concurrent current row. Note: this amends DECISIONS #49's presentation form only — military service is now a dated row on ABOUT while keeping its substance ruling (role description, no achievements) and EXPERIENCE's non-timeline inheritance.
- **Capabilities:** rebuilt as a scrolling logo wall via a new About-owned client component (`src/components/about/CapabilitiesWall.tsx`) — Home's shared `Capabilities.tsx` untouched. Three chip rows (one per cluster, same 25 confirmed items) travel in alternating directions relative to page scroll (row 1 L→R, row 2 R→L, row 3 L→R) driven by framer-motion scroll progress on transform-only `x`; travel clamped to measured overflow with a `min-w-[125%]` floor so short clusters still move (space distributed between chips). Brand chips reuse WORK's icon library and mappings (`YOLOv8`→Ultralytics, `Jetson Nano`→NVIDIA), recolored to muted with champagne on hover/focus-within only; methodology terms render identical text-only chips; Delta Lake/AWS marks verified absent from installed Simple Icons v13.15.1 (removed upstream) → text chips rather than a new icon source. Under `prefers-reduced-motion`: fully static wrapped grid at first paint (CSS media-query utilities + JS gating) — no clip/mask/transform, everything legible. Explicitly differentiated from Home's removed decorative marquee: content-bearing, bounded translation, all 25 items permanently in static DOM order for screen readers, no focusables inside tracks, no scroll interception.
- Verified: production build clean (`/about` static prerendered), `tsc --noEmit`, ESLint clean; headless suite against the production server 37/37 across desktop/tablet/mobile — zero horizontal overflow, 3 tracks / 25 chips / 10 icon chips per load, alternating directions confirmed from live computed transforms at two scroll positions, chip DOM count stable across scroll, zero console/page errors; reduced-motion emulation: rows wrap statically, transforms none, mask/clip removed, all 25 chips inside the content column before and after scrolling; rendered icon path data byte-compared against canonical Simple Icons sources (10/10 match — no placeholder icons); Education spec-grid, inline portrait, closing CTA, single H1, Home/`/work`/`/experience` regression-clean. Final visual sign-off manual by user.

Update (2026-08-23, second revision pass — Trajectory narrative spine + Capabilities continuous rows + row-2 motion-bug fix, user-directed; see DECISIONS #52):
- **Trajectory:** the prior pass's date-column list is superseded — user review found it read as a literal experience timeline, structurally duplicating EXPERIENCE's planned role-first format (PHASE3_UX §9), the exact conflation §8 forbids ("ABOUT frames the story; EXPERIENCE carries the timeline detail"). Rebuilt as a narrative spine: a static vertical hairline with a small ring node per phase transition (champagne node for the present), each phase carrying a small secondary mono date beside its node, a short story-beat label (never a job title), and 2–3 sentences of condensed prose. Six phases; FlyRank + Movenue merge into one closing "now" beat with the inline Movenue link. Zero factual loss — every figure/date spot-checked against source (see #52 list). Military service remains a role-description narrative (#49 substance intact); certifications remain contextual inline mentions (#50 intact). Spine/nodes/text fully static under reduced motion; per-phase entrance on the existing reduce-gated `Reveal`.
- **Capabilities:** root cause of the reported row-2 motion bug verified empirically before fixing — row 2 ("Computer Vision & Robotics") was the only row whose natural content fit INSIDE the column (979px vs ~1088px), so its motion depended entirely on the `min-w-[125%]` floor manufacturing overflow; whenever that utility was missing from compiled CSS (the documented Turbopack dev-cache case) maxX measured 0 and the row sat static while rows 1/3 moved on real overflow (plus `justify-around` slack gave it visibly wider chip gaps even when it did move). Fixed by construction: all 25 confirmed capabilities redistributed cluster-interleaved into three width-balanced (~1281/1208/1254px) header-less continuous rows so EVERY row overflows naturally at every breakpoint; the floor, `justify-around` slack, and the three mono cluster headers are removed so the section reads as one continuous logo-wall texture matching the reference structure (sr-only h3 preserves outline; cluster taxonomy stays on Home's Capabilities). Alternating scroll-linked directions kept; icon discipline unchanged.
- Verified: production build clean (`/about` static), `tsc --noEmit`, ESLint clean; headless suite vs the production server — 40+ SSR fact/date assertions incl. old-grid absence; live smooth-scroll frame sampling proves all three rows animate continuously (92–93 moving frames each, travels 193/121/166px, no jumps) with ordered-sample direction proof ltr/rtl/ltr; forced-reduced-motion instance shows zero transforms/wrapped static grids/no masks/all 25 chips visible inside the column/static readable spine; keyboard access to all content confirmed; zero console errors; Who I am, Education spec-grid, closing CTA, single H1, Home/`/work` regression-clean. Final visual sign-off manual by user.
