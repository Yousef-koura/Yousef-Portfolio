...
- **CONFIRMED** — PneumoScan / Chest X-ray pneumonia classification (5,863 X-rays; custom CNN vs ResNet-18, ONNX + FastAPI + React, May 2023/Apr 2026).
- **PROVISIONAL** — Old-site-only projects not in the resume: Ball Tracking Robot, Steganography Detector, standalone Breast Cancer AI repo, ML Projects collection, PotatoScan, SQL Projects. **User decision needed on inclusion** (source still says "verify before including").
