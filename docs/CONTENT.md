...
- **CONFIRMED** — Technical Office Engineer, License Department, Egyptian Armed Forces Engineering Authority (Cairo, Egypt; Jan 2025–Mar 2026, mandatory military service). **RESOLVED for ABOUT (2026-08-23, DECISIONS #49):** presented on `/about` as a narrative prose paragraph describing the actual role content (technical-office support, engineering documentation, licensing-department administrative processes, cross-function coordination) — not as a title/org/dates timeline entry. **RESOLVED for EXPERIENCE (2026-08-24, DECISIONS #53):** excluded from the `/experience` page ENTIRELY — no entry, no identity line, no footnote (explicit user decision, scoped to the EXPERIENCE page; the ABOUT presentation above is unchanged, and the earlier "EXPERIENCE inherits this ruling" expectation is superseded for this page).
